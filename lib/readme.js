...
function readme ($, url, callback) {
  var data = { "url" : url};
  //data.desc     = $('.repository-description').text().trim();
  //data.website  = $('.repository-website').text().trim();
  //var badges = $('.social-count');
  console.log($('p','#readme').text());
  //console.log(badges['0'].children[0].data.trim());
  //data.watchers = badges['0'].children[0].data.trim();
  //data.stars    = badges['1'].children[0].data.trim();
  //data.forks    = badges['2'].children[0].data.trim();

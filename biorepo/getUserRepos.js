...
var url = user.concat("?page=").concat(page).concat("&tab=repositories");
prepos(url,function(err,data){
	for(i=0; i<data.repos.length;i++){
		console.log(url.replace(/\?page..+/,"").replace("https://github.com/","")+"\t"+data.repos[i]+"\t"+data.flags[i]);
	}
})
